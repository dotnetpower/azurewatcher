...
def _runtime(**kwargs: object) -> PantheonRuntime:
    return PantheonRuntime.build(
        provider=InMemoryEventBus(), raw_event_topic=_RAW_TOPIC, **kwargs
    )
...
def test_ask_routes_to_primary_agent() -> None:
    ...
    turn = asyncio.run(
        runtime.ask(
            session_id="s1", user_id="u1", question="what is the action status"
        )
    )
...
def test_ask_tracks_session_turns() -> None:
    ...
    turn2 = asyncio.run(
        runtime.ask(session_id="s1", user_id="u1", question="approval backlog")
    )
...
def test_ask_enforces_user_ownership() -> None:
    ...
    with pytest.raises(PermissionError):
        asyncio.run(
            runtime.ask(session_id="s1", user_id="u2", question="action status")
        )
...
def test_conversational_port_absent_when_bragi_disabled() -> None:
    ...
    result = asyncio.run(
        runtime.ask(session_id="s", user_id="u", question="action status")
    )
...
def test_ask_handoff_when_no_route() -> None:
    runtime = _runtime()
    turn = asyncio.run(
        runtime.ask(session_id="s1", user_id="u1", question="zzzz qqqq wxyz")
    )
    assert turn is not None
