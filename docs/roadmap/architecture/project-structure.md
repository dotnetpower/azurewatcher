...
│   │   ├── read_api/           # thin ASGI - `main.py` composes one module per HTTP surface under `routes/`, including bounded evidence-preserving narrator prose review, SSE fan-out under `streaming/`, and separate `dev/` / `production/` wiring. GET routes project bounded state; POST command routes submit governed records or typed proposals and never call a privileged executor directly
