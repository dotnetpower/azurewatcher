...
translation_source_sha: a3a788de3b11eba42ab15f1108e0e847c98b5148
...
> base ref 와 working tree 의 catalog 를 materialize 해 거부된 전이가 있으면 빌드를 실패시킴. 남은
> 후속은 resolved assignment 를 소비하는 T0 런타임.
>
> 참고: ship 된 catalog-as-code 스키마는 아래 "YAML Shapes" 섹션(확장 가능한 목표)로 수렴 중.
> 공유 `Provenance` 값 객체
> ([`provenance.py`](../../../src/fdai/rule_catalog/schema/provenance.py))가 ship 되어 모든
> 아티팩트가 optional `provenance` 블록을 실고, 두 아티팩트 모두 `kind`
> ([`governance_kind.py`](../../../src/fdai/rule_catalog/schema/governance_kind.py)) discriminator 와
> 아티팩트 `version` 을 받음. 정규 `scope://` 주소 primitive
> ([`ScopeRef`](../../../src/fdai/rule_catalog/schema/scope.py))와 include/exclude
> [`ScopeBinding`](../../../src/fdai/rule_catalog/schema/scope.py) 폼 모두 ship 됨: assignment 의
> `scope` 는 단일 구조화 `level` + `id` 폼 또는 `scope://` `include` / `exclude` URI 리스트 +
> `selector` 폼이 될 수 있으며, 리솔버가 소비하는 `ScopeMatcher` 프로토콜 뒤로 통일됨. 아직
> 수렴 중인 나머지 간극: rule-set 은 `rule_set` / `target_rule_ids`(not `target`)로
> 바인딩되고, `parameters` 가 rule별 `parameter_overrides` 가 아닌 flat 형태임.
