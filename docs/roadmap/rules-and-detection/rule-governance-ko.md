...
translation_source_sha: 1944dc956d06aa1cec9581d702887eb3d28b0856
...
> ([`ScopeRef`](../../../src/fdai/rule_catalog/schema/scope.py))도 ship 됨 - full-ancestor-chain
> URI 를 parse / render 하고 `Scope` 로 브리지함. 아직 수렴 중인 나머지 간극: rule-set 은 `rule_set` /
> `target_rule_ids`(not `target`)로 바인딩되고, 로더는 아직 `scope://` include/exclude URI 리스트
> 대신 단일 구조화 `scope`(`level` + `id` + `excludes` + 타입된 `selector`) 를 받으며, flat
> `parameters`(not rule별 `parameter_overrides`)임.
