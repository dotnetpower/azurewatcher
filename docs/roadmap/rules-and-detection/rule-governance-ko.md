...
translation_source_sha: 11534ece446a2c85f9d3de2f061d57ddd253ab0f
...
> 아티팩트 `version` 을 받음. 아직 수렴 중인 나머지 간극: rule-set 은 `rule_set` /
> `target_rule_ids`(not `target`)로 바인딩되고, scope 는 `level` + `id` + `excludes` + 타입된
> `selector`(not `scope://` include/exclude URI) + flat `parameters`(not rule별
> `parameter_overrides`)로 모델링됨.
