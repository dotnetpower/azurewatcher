...
translation_source_sha: c4e3e11ac574311390b6c302a009addfda380765
...
> 위 값들은 placeholder zero - 실제 숫자는 [goals-and-metrics-ko.md](../architecture/goals-and-metrics-ko.md)
...
`fdai.agents.candidate_guard.CandidateGuard` 는 Mimir 가 모든 `RuleCandidate` 를 pending
...
- **Provenance** - `proposed_by` 와 알려진 `proposal_kind`
  (`new` / `revision` / `retirement` / `threshold_adjustment`) 가 필수.
