...
**Agent choreography.** Heimdall은 forecast finding과 결정론적 outcome closure를 소유하고
Huginn은 정규화된 실제 observation을 제공합니다. Saga는 변경 불가능한 prediction 및 terminal
evidence를 기록하고 Norns는 종료된 case-history cohort를 off-path에서 분석해 비활성
detector/rule candidate를 제안하며 Mimir는 검토된 promotion을 소유합니다.
...
돌아갑니다. Durable episode ledger, event-time outcome join, intervention censoring,
transactional publication outbox 및 기계적 tick 배선은 구현되어 있습니다. Promotion은 계속
측정된 deployment evidence와 authoritative promotion registry에 의존합니다.
