...
ranking을 수행합니다. Detector 및 metric filter도 failure 및 control cohort limit보다 먼저
적용되므로 더 최신인 관련 없는 detector record가 적격 case를 가릴 수 없습니다. Retriever는
제한된 case card와 source digest를 반환하며 모델은 embedding을 source evidence로 취급할 수
없습니다.
