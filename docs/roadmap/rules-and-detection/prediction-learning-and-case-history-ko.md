...
Case는 append-only revision을 가진 안정 identity입니다. Incident가 reopen되거나 늦은 trusted
evidence가 도착하면 history를 덮어쓰지 않고 revision을 추가합니다.
...
percent-encoded credential 형태도 거부합니다.
...
Retrieval은 검색 전에 purpose 및 access scope를 authorize합니다. Resource type, metric,
...
가집니다. Deletion은 hot index를 tombstone하기 전에 artifact, chunk 및 embedding을 제거합니다.
Audit에는 non-sensitive deletion record와 digest를 유지합니다. Artifact deletion 실패는 retryable
상태로 남고 완료로 표시되지 않습니다. 기계 scheduler는 primary event bus에 제한된 raw retention
tick을 publish합니다. Huginn이 이를 정규화하고 Muninn만 typed `object.event` retention signal을
소비해 due deletion을 적용합니다. `FDAI_CASE_HISTORY_RETENTION_TICK_SECONDS`는 cadence를 제어하며
기본값은 1일입니다. 중복되거나 replay된 tick은 idempotent합니다. Raw event가 전달하는
