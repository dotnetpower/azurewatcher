...
- **소스 ACL provenance.** 소스 문서를 *누가 읽을 수 있었는지*를 `provenance`에 기록한다.
  ...
  본문으로 유출해서는 안 됩니다. Stable machine-record key는 English를 유지하고 human-facing
  prose는 configured locale을 따를 수 있으며 모든 surface는 secret-free를 유지합니다
...
- **민감도 게이트.** 서비스 계정이 *읽을 수 있는* 문서라도 무턱대고 증류하면 안 되는 것일 수
  ...
  민감도 hold는 의도적으로 freshness 스냅샷에 기록하지 않으므로, 미해결 비밀은 첫 패스 후
...
키잉한다. 그래서 페이지에 `runbook` 라벨을 다시 붙이거나 verified로 표시하면 텍스트가
...
마운트 실패나 인증 만료와 구별되지 않으므로, 수상 소스 장애로 보고 fail-closed한다
...
Model call은 offline candidate extraction과 source-fidelity validation(back-translation 및
mixed-model comparison)에 한정되고 이벤트마다가 아니라 **매뉴얼 리비전당 한 번** 실행됩니다.
그 뒤 structural, shadow 및 regression gate는 결정론적 검증 + human review입니다.
...
| Back-translation | 아직 구현되지 않은 backlog | - |
