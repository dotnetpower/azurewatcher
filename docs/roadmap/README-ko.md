...
| # | 문서 | 다루는 내용 |
|---|------|-------------|
| 24b | [reporting-subsystem-ko.md](interfaces/reporting-subsystem-ko.md) | 선언적 시각화 파이프라인: YAML 리포트 카탈로그, datasource / widget / format 레지스트리, 기존 seam 위에 등록되는 widget builder와 datasource adapter, 확장 가능한 encoder, 읽기 전용 `GET /reports/*` route, fork 확장 recipe. Registry가 늘어나도 backend-only contract는 안정적으로 유지. |
