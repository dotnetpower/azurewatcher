...
> [operator-console.md](operator-console-ko.md)에 문서화. Push와 pull은
> 채널 credential + routing config를 공유하지만 send-only와
> receive-plus-send blast-radius를 별개로 유지하기 위해 별개 adapter로
> ship.
...
- 어댑터는 `delivery/chatops/<vendor>/` 아래 존재
  ([project-structure-ko.md](../architecture/project-structure-ko.md) 참조). 각 어댑터는 `shared/providers/`의
