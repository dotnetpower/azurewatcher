...
>
> **구현 상태.** 전체 lifecycle이 구현되었습니다. Production map binding, fail-closed
> Terraform identity injection, scheduled stale-OID transition audit, grounded handover ingestion,
> idempotent GitHub draft-PR delivery, signed merge observation, stakeholder notification,
...
5. **콘솔은 map을 직접 변경하지 않는다.** 스튜어드십 projection은 read-only를 유지합니다.
  Guided registration form은 structured `handover_bootstrap` document를 ingestion boundary에
  제출하고, GitHub App은 다른 모든 governance change와 동일하게 결과를 draft PR로 작성합니다
...
아래 lifecycle은 구현되었습니다. Ingestion gateway는 grounded handover draft를 idempotent
governance PR로 변환합니다. Signed GitHub webhook은 changed file과 merged YAML을 다시 읽고 core
recipient 및 audit primitive로 lifecycle을 완료합니다.
...
3. **감사.** Saga append-only `AuditEntry`가 actor identity, 아티팩트, before -> after 요약, correlation
...
- **담당자 등록** - Contributor, Approver, Owner는 canonical agent, 사람 또는 group의 display name이나
  ...
  제출합니다. Reader에게는 locked 안내를 표시합니다.
- **검토 결과** - 처리된 draft는 mapping, unresolved identity, unmapped agent count, generated YAML,
  governance delivery가 enabled일 때 idempotent draft PR link를 표시합니다.

SPA는 Git credential을 보유하지 않고 `config/agent-stewardship.yaml`을 직접 쓰지 않습니다.
Ingestion service가 identity를 resolve하고 core resolver로 additive candidate를 validate한 다음 Owner가
검토할 draft PR을 생성합니다. Loader는 maintainer 1명 미만을 거부하고 console은 2명 미만에 권장
banner를 표시합니다.

## 10. 보안과 안전

- 스튜어드십은 executor Managed Identity를 보유하거나 부여하지 않는다.
- Console button은 proposal을 제출할 수 있지만 map 변경에는 감사되는 governance PR
  (author != approver)과 merge가 계속 필요합니다.
...
polling하고 검토용 draft summary, YAML, persisted governance PR receipt를 렌더링합니다. Map을
...
플레이스홀더 id 포함). Stewardship governance가 enabled이면 delivery layer는 그 YAML을
idempotent governance draft PR 하나로 publish합니다. 사람이 검토하고 merge하면 signed merge
webhook이 merge audit를 작성하고 affected owner에게 알립니다. Console은 source proposal을
제출할 수 있지만 resulting map을 적용할 수 없으며 어떤 map도 autonomous apply하지 않습니다. 전체
state, failure, deployment contract는
