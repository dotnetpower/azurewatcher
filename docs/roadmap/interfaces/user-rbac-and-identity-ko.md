...
Settings activity bar 그룹은 콘솔의 클라우드 권한을 넓히지 않고 네 개의 안정적인 경로를
제공합니다.

| 경로 | 목적 |
|------|------|
| `/settings/general` | 브라우저 로컬 표시, 언어, 모션 및 답변 검증 환경 설정입니다. |
| `/settings/models` | 해결된 T1/T2 모델, 자동 라이프사이클 상태, 지연 시간 근거 및 로그인 사용자의 T1 narrator 선호입니다. |
...
받고 콘솔은 잠긴 surface를 렌더링합니다. 역할이 없는 사용자는 role-optional
`GET /iam/self` projection을 통해 자신의 요청만 봅니다.
...
Azure CLI 자격 증명과 구성된 다섯 `FDAI_RBAC_*_GROUP_ID` 값을 사용해 같은 Microsoft Graph
adapter를 호출합니다. Synthetic 사용자로 fallback하지 않으므로 alias 검색, 역할 roster 및
access request 대상은 로그인한 tenant의 실제 데이터를 반영하며 provider 자격 증명은
브라우저 외부에 유지됩니다.
