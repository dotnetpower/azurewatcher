...
- **LLM 비용 (T2 추론)**: **인프라 envelope에서 제외** 되며 [T2 LLM Cost](#t2-llm-cost) 에
  별도 보고. LLM 지출은 [llm-strategy-ko.md](../architecture/llm-strategy-ko.md) 의 모델 예산 상한에 의해
...
Reasoning-tier (T2) 추론은 **Azure 리소스 라인이 아님** - [llm-strategy-ko.md](../architecture/llm-strategy-ko.md)
...
| **Container Apps min-replicas = 0 도처에** | 이미 기본값; 유지 | 콜드스타트 지연은 [operating-and-verification-ko.md](../operations/operating-and-verification-ko.md#self-health-signals) 로 계산 |
