...
Production 브라우저는 전용 ingestion gateway에 인증된 bounded streaming PUT을 보내고 gateway가
전용 managed identity로 private object storage에 기록합니다. Browser에 storage credential이나
executor identity를 주지 않습니다. 이후 이벤트 기반 파이프라인이 파일을 검사하고, 분류하고,
...
message 또는 tool argument에 추가되지 않습니다.
...
1. **Uploading:** 현재 UI는 hashing/uploading 상태와 cancel을 표시하고 파일 하나를 한 번의 bounded
  PUT으로 전송합니다. Byte progress, pause/resume, block checkpoint는 future provider capability입니다.
...
`complete` 이후 브라우저를 닫아도 서버 처리는 취소되지 않습니다. 현재 Console은 열린 view에서
상태를 polling하며 durable activity/history는 upload id로 조회할 수 있습니다. 취소하면 새 작업을 중단하고 session을 폐기하며
...
decrypted copy를 재사용하지 않습니다. Rights-managed file의 경우 production deployment는 source
...
2. Gateway가 collection과 policy를 authorize하고 quota를 예약한 후 해당 session의 인증된
  `/content` target과 expiry를 반환합니다. Storage credential은 browser에 반환하지 않습니다.
3. 현재 Console은 파일을 순차적으로 gateway에 한 번의 PUT으로 전송합니다. Gateway는 ADLS로
  stream하며 size와 SHA-256 metadata를 봉인합니다.
4. Client가 `complete`를 호출하면 gateway가 object property, expected size/hash를 확인합니다.
5. Gateway가 session을 닫은 후 `document.received`를 publish합니다.

현재 retry는 같은 upload session의 target을 다시 얻을 수 있지만 content PUT은 처음부터 다시
전송합니다. Block checkpoint와 browser-restart resume은 향후 object-store provider capability입니다.
Gateway identity는 예약된 opaque object key에만 쓰고 다른 문서를 browser에 list/read하지 않습니다.
...
각 production deployment는 측정된 baseline을 수립하고 다음 항목에 대한 p50/p95 목표를 설정합니다.
...
Architecture는 bounded gateway streaming, 동일 security scope 안의 content-hash deduplication,
...
문서가 retrieval 또는 distillation에 참여할 수 있는 상태는 `ready` 또는 policy가 허용한
...
Quarantine promotion 전에 ADLS adapter는 governed HNS parent directory를 각각 idempotent하게
생성합니다. Rename response가 유실된 경우 retry는 governed target이 존재하면 성공으로
처리합니다. Source와 target 어느 쪽으로도 promotion을 완료할 수 없을 때만 missing source를
보고합니다.
...
|-----------------|---------|
| `GET /healthz` | deployment verification용 unauthenticated process liveness, `{"status":"ok"}`만 반환 |
...
| `POST /ingestion/uploads/{upload_id}/resume` | 권한과 session state를 재검사하고 현재 upload target 반환 |
| `PUT /ingestion/uploads/{upload_id}/content` | 인증된 bounded source stream을 ADLS quarantine에 기록 |
| `POST /ingestion/uploads/{upload_id}/complete` | 수신한 object를 verify하고 commit |
| `GET /ingestion/uploads/{upload_id}` | 권한이 적용된 upload-session과 processing status |
...
Source byte는 client에서 전용 gateway를 거쳐 object storage로 streaming됩니다. Authentication
token은 header로 전달하며 storage credential이나 grant를 query string으로 browser에 노출하지 않습니다.
...
|---------|---------------|
| Browser 또는 network disconnect | 실패한 streaming PUT의 partial object를 삭제하고 같은 session에서 처음부터 retry하거나 abandoned session을 expire합니다. |
...
2. **Safe text slice:** gateway streaming upload, quarantine, malware scan, plain-text extractor,
...
- Production browser는 scoped session의 authenticated gateway target으로 upload하며 gateway가
  private object storage로 stream합니다. Direct-to-storage grant는 future provider capability입니다.
...
- 현재 bounded upload는 streaming 방식입니다. Sharded/block-resumable large-document processing은
  production shipped 기능이 아니라 provider target입니다.
