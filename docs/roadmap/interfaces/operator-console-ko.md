...
translation_source_sha: 1d493ae339dde3712b9e65a00b7d5901ee662bdf
...
방식 — CLI, Teams, Slack, 웹 챗 전반. 이 문서는
...
agent, provisioning SSE reader는 keepalive comment를 포함해 45초 동안 byte가 없으면 연결을 cancel하고
기존 bounded reconnect path로 이동합니다. Provisioning은 event 전달이 실패해도 reader를 cancel합니다.
Canvas
...
Architecture는 subscription scope cached snapshot이 있으면 즉시 렌더링합니다. 만료되었거나 change로
invalidate된 snapshot은 stale로 표시하고 refresh 진행 상태를 보여 주며, read API가 완료된 background
refresh를 원자적으로 promote할 때까지만 polling합니다. Cache 상태는 server-owned freshness verdict를
상향하지 않습니다. Polling은 stale graph를 계속 사용할 수 있게 유지하면서 transient failure를
2-30초 범위의 bounded backoff로 재시도합니다.
