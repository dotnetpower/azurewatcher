...
- [`src/fdai/core/conversation/channel_gateway.py`](../../../src/fdai/core/conversation/channel_gateway.py)
  - injected principal resolver로 sender를 인증하고, bounded session id를 만들고, message
    idempotency key를 claim하고, 기존 coordinator를 호출하고, 응답을 원래 thread로 route합니다.
...
- [`src/fdai/delivery/read_api/routes/scheduler_runs.py`](../../../src/fdai/delivery/read_api/routes/scheduler_runs.py)
  및 [`console/src/routes/scheduler-runs.tsx`](../../../console/src/routes/scheduler-runs.tsx)
  - reader-role `GET` panel과 read-only Operations view로 task-scoped scheduler dispatch 근거를
    제공합니다. Route는 status filter와 opaque cursor pagination을 지원하지만 retry, cancel,
    execute, approval control은 없습니다.
...
Production composition에는 concrete Bot Framework JWT verifier, channel principal resolver, app
credential, background gateway lifecycle binding이 아직 필요합니다. 이 binding은 `delivery/`에
유지되며 coordinator를 변경하지 않습니다.
...
|---------|-----------------|-----------------|---------------|
| Teams | `TeamsHilAdapter` (Incoming Webhook 또는 Bot Framework send로 Adaptive Card) | `TeamsBotChannel` + bounded activity route + workload-identity reply publisher, concrete JWT/principal composition은 남음 | Tenant, 채널 id, app registration |
| Slack | `SlackWebhookChannel` (Incoming Webhook로 Block Kit) | `SlackBotChannel` + signed Events API route + fixed-endpoint Web API reply publisher, app composition은 남음 | Workspace, 채널 id, app credential |
...
Capabilities route는 inert catalog projection입니다. Response는
`source=static-catalog`와 `execution_eligibility=false`를 식별하며 entry는 선언된
side-effect class, required role, default mode를 설명합니다. Catalog presence는 provider
binding, runtime health, execute permission을 증명하지 않습니다. 이 결정은 composition,
RBAC, verification, risk gate에 남습니다.
...
유지하지만 Engaged, Watching, Idle은 agent stream이 open일 때만 current count입니다. Canvas
visualization은 동등한 keyboard 및 screen-reader resource selector를 제공하고 composite tab widget은
roving selection과 함께 DOM focus를 이동합니다.
...
이 outbound catalog는 FDAI 자체를 MCP server로 publish하는 것과 구분됩니다. Upstream console은
Day 1에 해당 inbound MCP server를 ship하지 않습니다. In-process tool set과 RBAC matrix가
안정되면 이후 동일한 tool catalog (`list_tools` / `call_tool`) 및 operator-console read resource를
MCP resource로 publish할 수 있습니다.
