...
translation_source_sha: a77cb8deec0cfa97d16a17a007b254f5c280d1e1
...
| `deny` / `remediate` | `audit` | 표준 리뷰 (강등은 항상 허용 — 안전 방향으로 실패) |
...
- 스코프는 데이터; executor는 여전히 최소권한 아이덴티티와 액션 화이트리스트만 보유
  ([security-and-identity-ko.md](security-and-identity-ko.md)) — 광범위 스코프는 실행 권한을
  절대 넓히지 않음.
...
- **별개 approver**: requester는 approver가 되어선 안 됨 (no self-override), exemption 규칙과
  [security-and-identity-ko.md](security-and-identity-ko.md) 의 approval≠execution 경계 미러링.
...
[security-and-identity-ko.md](security-and-identity-ko.md) 의 approval≠execution 규칙
미러링. 이들은 **논리적** 거버넌스 롤; 소수의 Entra 보안 그룹(Reader / Contributor / Approver
/ Owner + Break-Glass) 에 매핑 ([user-rbac-and-identity-ko.md](user-rbac-and-identity-ko.md)).
...
[user-rbac-and-identity-ko.md#conditional-access](user-rbac-and-identity-ko.md#43-conditional-access)).

**Risk-classification 테이블** ([risk-classification-ko.md](risk-classification-ko.md)) 은
