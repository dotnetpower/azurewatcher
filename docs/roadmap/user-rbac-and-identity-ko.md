...
translation_source_sha: fc8fb7a4e0b16eda39a5e89c8a34d291831bbe2f
...
Managed Identity, GitHub App, Teams bot)는 여전히 [security-and-identity-ko.md](security-and-identity-ko.md) 와
[deploy-and-onboard-ko.md](deploy-and-onboard-ko.md) 가 관장.
...
|------------------|------|---------|------|
| `aiopspilot-console-spa` | SPA (PKCE, secret 없음) | `aiopspilot-api` 스코프 요청 | 콘솔 사인인만 |
| `aiopspilot-api` | Web API | `api://<guid>` | 콘솔 + ChatOps backend가 호출; **App Roles** (§4.4) 선언, 모든 요청의 `roles` claim 검증 |
| `aiopspilot-approval-bot` | Bot (Azure Bot channel registration) | `aiopspilot-api` on-behalf-of Teams SSO | Adaptive Card HIL 승인 |
...
API는 원시 그룹 claim이 아니라 **App Roles** 로 authorize. App Roles는 `aiopspilot-api` app
...
- **앱-스코프 최소권한.** App Roles는 `aiopspilot-api` 에만 적용; 손상된 토큰의 blast
...
sequenceDiagram
  ...
  participant API as aiopspilot-api
...
| `aiopspilot-api` 액션 로그 | 어떤 API 호출, `justification`, `entra_oid`, `correlation_id` |
| GitHub PR 이벤트 | PR 저자 trailer, 리뷰어 승인, CI 검사 결과 |
| `core/audit` | 최종 결정, 티어, executor / 승인자 아이덴티티, idempotency 키 |

상관 ID는 흐름의 첫 사용자-개시 액션에서 `aiopspilot-api` 가 생성하고 GitHub(PR body),
...
2. **오디언스** 가 `api://<aiopspilot-api-guid>` 와 같음.
...
- 봇은 Teams 토큰을 `aiopspilot-api` 오디언스 토큰으로 교환하는 **On-Behalf-Of (OBO)**
  플로우 실행.
...
- **로컬 dev**: `az login --use-device-code` 를 통한 device code 플로우, dev 테넌트에 대해
  전용 `aiopspilot-api-dev` 오디언스에 범위됨.
- **CI**: workload identity federation (OIDC), [deployment-ko.md](deployment-ko.md) 에서
...
- Break-glass 자격증명 로테이션과 드릴 주기는
  [security-and-identity-ko.md](security-and-identity-ko.md) 에 선언.
...
- [ ] 콘솔의 "draft change" UI가 P1(Change Safety만)에 실릴지 P3(3개 버티컬 모두)에 실릴지
      — [rule-governance-ko.md](rule-governance-ko.md#open-decisions) 저작-UI 결정에 의존.
