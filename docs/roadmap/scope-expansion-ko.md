...
translation_source_sha: 6b552cf7370dd5800980820183d26772304deb54
...
| SLO / error budget | Partial | `core/slo/`: `MetricBurnRateSource` 가 § 3.2 metric seam 을 burn-rate evaluator 에 브리지(데이터 누락 / 불일치 시 fail-closed); real vendor `MetricProvider` adapter + 스케줄링된 평가 남음 |
| Monitoring / alerting (외부 signal ingestion) | Partial | `core/detection/` correlation 은 ship; § 3.2 metric / log / trace Protocol + in-memory 바인딩 존재, real vendor adapter 는 미배선 |
...
그리고 그것을 consume 하는 `core/slo/` 브리지가 모두 존재합니다; 구체 backend 와
스케줄링된 평가 루프만 남았고, 둘 다 `core/` 재작성 없이 additive 로 land 합니다(fork
adapter + phase task). `Deferred` 행은 컨트롤 루프의 gap 이 아니라 설계상 seam 입니다.
