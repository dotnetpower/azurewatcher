...
조용한 실패 모드가 있습니다. 별도로 권한을 부여한 topic의 주기적 canary로 완화합니다.

- **합성 이벤트**는 Container Apps Job에서 5분마다 같은 Event Hubs namespace의
  `aw.control.canary`로 게시됩니다.
- 전용 UAMI는 이미지를 pull하고 Event Hubs에 send할 수만 있습니다. 코어의 별도 canary consumer는
  `source=fdai.canary-job`과 `event_type=fdai.control.canary`만 허용합니다.
- Canary 경로는 ingest, route, audit stage와 no-op audit entry를 기록합니다. T0/T1/T2, risk gate,
  execution, IRP, learning loop에는 진입하지 않습니다.
...
- 각 5분 slot은 stable UUID와 `canary:<slot>` idempotency key를 생성합니다. Container Apps는
  publisher 실행을 120초로 제한하고 audit row는 측정된 latency를 기록합니다.

> 배포 workflow는 즉시 실행한 canary publisher가 실패하면 차단됩니다. Audit freshness blocking
...
현재 apply workflow는 schema migration, 선택적 HTTP health endpoint, 성공한 canary publisher
Job을 검증합니다. 전체 audit round-trip, fixture replay, kill-switch drill, HIL dry-run은 production
promotion 전에 여전히 필요합니다.
