...
[deployment-preflight](../deployment/deployment-preflight.md) pass and the
...
|------------------|-----------------|--------------------------------|
| [deployment-preflight](../deployment/deployment-preflight.md) | one deploy: can this change land in the target scope | scoped to a single `terraform apply` / remediation PR, not the accumulated posture |
...
same scope hierarchy the [rule-governance](../rules-and-detection/rule-governance.md) overrides use),
...
|-----------|----------------------|--------------|
| `policy_guardrail` | disallowed resource types, public access, missing encryption | [rule-catalog-collection.md](../rules-and-detection/rule-catalog-collection.md) |
...
[deployment-preflight](../deployment/deployment-preflight.md) `blocks_deploy` flag uses.
...
promotion discipline the [ActionType contract](../architecture/llm-strategy.md) and the
...
[risk-classification.md](../decisioning/risk-classification.md) and never auto-execute. The ORR
