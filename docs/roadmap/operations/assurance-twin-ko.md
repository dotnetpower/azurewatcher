...
[observability-and-detection-ko.md](../rules-and-detection/observability-and-detection-ko.md) 의 탐지 finding,
...
보안 범위 리포트는 severity-only finding 목록보다 더 많은 맥락을 유지합니다.
Collector는 Azure Resource Graph 속성, 서버 parameter, Defender assessment, WAF
record, policy compliance, diagnostic setting, version/advisory match를
`SecurityControlObservation` 값으로 정규화합니다. 각 observation은 현재값과 기대값,
...
| Source data | 추출하는 정보 |
|-------------|---------------|
| Azure Resource Graph resource property | AKS version, private API, RBAC, network policy, Entra/local-account 상태, workload identity, image cleaner, add-on, upgrade channel과 MySQL network, backup, HA, encryption, version |
| Azure Resource Graph `sku` 및 `kind` | AKS와 MySQL service tier 및 resource kind |
| AKS node-pool resource property | Node image version, secure boot, virtual TPM |
| MySQL server parameter | Secure transport, 허용 TLS version, audit logging |
| Azure Monitor diagnostic setting | 승인된 platform log와 metric이 evidence store로 routing되는지 여부 |
| Defender for Cloud assessment | Runtime protection coverage와 actionable unhealthy finding |
| Application Gateway WAF log | Matched/blocked rule, attack detail, resource, event evidence |
| Security bulletin 및 advisory match | CVE id, 적용 가능성, patch 상태, source URL, managed-service backport note |
| Rule 및 compliance metadata | 기대값, rationale, remediation, validation, priority, 조치 기한, compliance control |
| Report-feed timestamp 및 source error | Evidence window, source availability, partial read, freshness gap |

관측된 ARG 또는 ARM inventory snapshot이 promote되면 inventory job은 bounded row
cap 아래에서 active AKS, node-pool, MySQL record만 읽고 결정론적 Azure analyzer를
실행한 다음 timestamp가 있는 control signal을 durable report feed에 기록합니다.
Supplemental provider는 server parameter, diagnostic-setting 상태, Defender coverage,
advisory match를 추가할 수 있습니다. Provider가 구성되지 않았으면 해당 control과
source coverage는 control 실패가 아니라 `unknown` 또는 `unavailable`로 남습니다.

`build_security_assessment`는 순수하고 결정론적인 fold로 유지됩니다. Verdict와 함께
다음을 파생합니다.

- finding, rule, resource, resource type, control, evidence 개수
- pass, fail, warning, not-applicable, unknown control 개수
- control pass rate, evidence coverage, source coverage
- category 및 resource type 분포
- positive control과 unknown control
- due timestamp와 검증 단계를 포함한 우선순위별 recommendation
- CVE 적용 가능성, patch 상태, compliance mapping
- available, partial, unavailable, stale data source 개수

`clear` verdict는 관측된 위험만 설명하며 assessment가 완전하다는 뜻은 아닙니다.
`completion_status`, source coverage, stale source, unknown control, missing evidence를
계속 표시하므로 provider를 사용할 수 없는 상태가 잘못된 clean 결과로 바뀌지 않습니다.

읽기 전용 `Security Assessment` catalog report는 기존 Reports 페이지에서 이 projection을
렌더링합니다. KPI, control-status, chart, table, group, tabs, note widget을 재사용하므로
새 browser 실행 표면이나 특권 identity를 도입하지 않습니다.
...
| 리뷰가 소비하는 탐지 finding | [observability-and-detection-ko.md](../rules-and-detection/observability-and-detection-ko.md) |
