...
|------------------|------|----------|-------|
| `aiopspilot-console-spa` | SPA (PKCE, no secret) | requests `aiopspilot-api` scopes | Console sign-in only |
| `aiopspilot-api` | Web API | `api://<guid>` | Called by console + ChatOps backend; declares **App Roles** (§4.4) and validates the `roles` claim on every request |
| `aiopspilot-approval-bot` | Bot (Azure Bot channel registration) | Teams SSO on-behalf-of `aiopspilot-api` | Adaptive Card HIL approvals |
...
`aiopspilot-api` app registration and assigned to the `aw-*` groups in the Enterprise
...
- **App-scoped least privilege.** App Roles apply only to `aiopspilot-api`; they cannot
...
| `aiopspilot-api` action log | which API call, with `justification`, `entra_oid`, `correlation_id` |
...
Correlation ID is generated by `aiopspilot-api` on the first user-initiated action of a
