...
|------------------|------|----------|-------|
| `azurewatcher-console-spa` | SPA (PKCE, no secret) | requests `azurewatcher-api` scopes | Console sign-in only |
| `azurewatcher-api` | Web API | `api://<guid>` | Called by console + ChatOps backend; declares **App Roles** (§4.4) and validates the `roles` claim on every request |
| `azurewatcher-approval-bot` | Bot (Azure Bot channel registration) | Teams SSO on-behalf-of `azurewatcher-api` | Adaptive Card HIL approvals |
...
`azurewatcher-api` app registration and assigned to the `aw-*` groups in the Enterprise
...
- **App-scoped least privilege.** App Roles apply only to `azurewatcher-api`; they cannot
...
sequenceDiagram
  ...
  participant API as azurewatcher-api
...
| `azurewatcher-api` action log | which API call, with `justification`, `entra_oid`, `correlation_id` |
...
Correlation ID is generated by `azurewatcher-api` on the first user-initiated action of a
...
2. **Audience** equals `api://<azurewatcher-api-guid>`.
...
- The bot performs the **On-Behalf-Of (OBO)** flow to exchange the Teams token for an
  `azurewatcher-api` audience token.
...
- **Local dev**: device code flow via `az login --use-device-code`, scoped to a dedicated
  `azurewatcher-api-dev` audience against a dev tenant.
