...
> [project-structure.md](project-structure.md) but never edits the pantheon
> itself
...
- **`docs/roadmap/agent-workflows.md` (+ ko)** - the 10 cross-agent
  workflows with sequence diagrams and exit criteria. See §5 of this doc
  for the workflow inventory.
- **`docs/roadmap/agent-pantheon.md` §4 detail** - each of the 15 agents
...
- New package `src/fdai/agents/` with:
  ...
  - `pipe.py` - a thin adapter that wraps the existing event bus
    provider protocol (see [project-structure.md](project-structure.md#customization-via-dependency-injection))
...
- **Saga (`src/fdai/agents/saga.py`)** - implement `append_audit`
  ...
  [security-and-identity.md](security-and-identity.md)). Implement
...
- **Bragi (`src/fdai/agents/bragi.py`)** - conversational port entry
  ...
  ([operator-console.md](operator-console.md)) reused. Implement:
...
- **KPI collectors** - each agent emits its declared KPIs (see
  ...
  pipeline ([goals-and-metrics.md](goals-and-metrics.md)).
...
- **Model fine-tuning.** LLM strategy and fine-tuning are governed
  by [llm-strategy.md](llm-strategy.md); the pantheon uses whatever
...
| ActionType schema referenced by W0 | [action-ontology.md](action-ontology.md) |
| KPI measurement pipeline referenced by W8 | [goals-and-metrics.md](goals-and-metrics.md) |
| Fork seams referenced by W2, W5, W6 | [project-structure.md](project-structure.md#customization-via-dependency-injection), [downstream-fork-guide.md](downstream-fork-guide.md) |
| Existing standard-set wave plan (for style reference) | [implementation-plan.md](implementation-plan.md) |
