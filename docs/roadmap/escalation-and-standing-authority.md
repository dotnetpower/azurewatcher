...
- A `hil` verdict fires an approval request with a TTL. Today, TTL expiry is a
  **no-op + audit + A2 alert** ([channels-and-notifications.md § on-call,
  escalation, timeouts](channels-and-notifications.md)). Fail-closed, correct -
  but it stops there.
- **Channel fallback** already exists: a failed Teams approval falls to another
  A1-capable channel, then pages the ops lane
  ([channels-and-notifications.md](channels-and-notifications.md)). That handles
...
- A **forecast finding** carries a shrinking **lead time** (`actual_breach_time
  ...
  Predictive / Forecasting](observability-and-detection.md#3-predictive--forecasting)).
...
|---------|---------|---------------|----------|
| **Channel fallback** | delivery failure | channel unreachable / send error | [channels-and-notifications.md § 6](channels-and-notifications.md) |
...
- **No self-approval survives escalation.** A later rung is a *different*
  ...
  [agent-pantheon.md](agent-pantheon.md)).
- **Every rung transition is audited** and, when the fingerprint repeats, feeds
  ...
  ([agent-pantheon.md § 6.4 Handoff escalation protocol](agent-pantheon.md)).
...
- **Inputs** (all already produced upstream, no new collection): `impact` /
  ...
  ([observability-and-detection.md § 3](observability-and-detection.md#3-predictive--forecasting)),
...
3. **Metrics** (fold into the existing KPI stream,
   [goals-and-metrics.md](goals-and-metrics.md)): rung-response latency,
...
| Forecast lead time and the prediction-interval band | [observability-and-detection.md § 3](observability-and-detection.md#3-predictive--forecasting) |
| Channel fallback vs this human-authority ladder | [channels-and-notifications.md](channels-and-notifications.md) |
| Which agent escalates, judges, and executes | [agent-pantheon.md](agent-pantheon.md) |
