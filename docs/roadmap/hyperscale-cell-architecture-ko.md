...
**정책-기반 fan-in**, **CQRS 감사 인덱싱**을 더해서, 어떤 안전 불변식도 깨지
않고 초대규모로 유입/인덱싱/조회를 처리한다.

> **범위:** 이 문서는 day-zero 배포가 아니라 **향후를 내다본 scale-out
> 설계**다. [deploy-and-onboard-ko.md](deploy-and-onboard-ko.md) 의 최소비용
> 토폴로지(Event Hubs Standard 1 TU, 사이드카를 가진 단일 Container App,
...
- **correlation 이 감사 평면을 줄인다.** `EventCorrelator`
  ([observability-and-detection-ko.md § 1](observability-and-detection-ko.md#1-이벤트-상관관계event-correlation))
...
의 단일앱-플러스-사이드카 형태가 **셀 1개**다. B안은 여러 개를 돌린다.
...
- **Warm**(수개월): ADLS Gen2 컬럼나(Parquet) - 저렴한 스캔과 집계.
...
초대규모 프로파일 모두(KEDA scale-to-zero, 최저 운영부담). 사이드카를 가진 단일
Container App 을 셀별로 샤딩하면 **약 10-16k events/s** 를 지속 처리한다(Event
Hubs 파티션 32개 x replica 당 수백 결정/s); ~40k events/s Standard 천장까지의
버스트는 파티션 버퍼가 흙수하고 lag 으로 소진한다 - 무손실. `max_replicas` 상한은
...
가격 매기는 [cost-model-ko.md](cost-model-ko.md) 를 확장한다. `sovereign` 프로파일은
...
> [cost-model-ko.md § T2 LLM Cost](cost-model-ko.md#t2-llm-cost) 에 별도 보고된다.
...
| 최소셋 잔여 (Key Vault, ACR, Bot, ...) | - | **$30 - $40** | [cost-model-ko.md](cost-model-ko.md) 에서 이월 |
...
| 학습 대상 | 읽을 문서 |
|-----------|-----------|
| 두 로그 평면과 감사-쓰기 병목 | [observability-and-detection-ko.md](observability-and-detection-ko.md) |
| 이것이 보존하는 8개 wire-level 계약 | [csp-neutrality-ko.md](csp-neutrality-ko.md) |
| 이것이 scale-out 하는 최소비용 토폴로지 | [deploy-and-onboard-ko.md](deploy-and-onboard-ko.md) |
