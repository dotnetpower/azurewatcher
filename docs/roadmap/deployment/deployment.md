...
- Provisioned resources - **minimum cost-efficient set** (full inventory + tier decisions in
  ...
  inventory renders the five wire-level contracts in [csp-neutrality.md](../architecture/csp-neutrality.md)):
  ...
  - Free-tier / non-billable elements: Static Web Apps (console), Azure Bot (HIL Adaptive
    Cards), workload identity federation (CI/CD), app registrations for console SPA + API + approval bot ([user-rbac-and-identity.md](../interfaces/user-rbac-and-identity.md)).
...
- [ ] Terraform-only vs Terraform + Bicep split (see [tech-stack.md](../architecture/tech-stack.md)).
