...
translation_source_sha: 1246e3c6555bfc102604506eea93ce83c7fe1303
...
### Compute Shape (단일 모듈식 프로세스)

코어는 현재 하나의 Container App 안에서 서명된 이미지 하나와 Python 프로세스 하나로
배포됩니다. 코드 수준의 서브시스템 경계는 Protocol과 composition root로 유지되며, 문서화되지
않은 localhost IPC 또는 sidecar 이미지 수명 주기는 없습니다.
...
| `AZURE_RG` | env | deployment | 대상 리소스 그룹 |
...
| `KEYVAULT_URL` | env | deployment | executor MI가 시크릿에 GET |
...
| `TEAMS_HIL_CHANNEL_ID` | env | deployment | HIL 라우팅 |
| `T2_MODEL_ENDPOINT` | env | deployment | primary reasoner - `rule-catalog/llm-registry.yaml` 에서 부트스트랩 resolver가 채움; [llm-strategy-ko.md § Model Provisioning and Lifecycle](../architecture/llm-strategy-ko.md#model-provisioning-and-lifecycle) 참조 |
| `T2_MODEL_ENDPOINT_CROSSCHECK` | env | deployment | mixed-model 교차 검사 대상 - 부트스트랩에서 별개 publisher 강제 |
