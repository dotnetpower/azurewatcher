...
translation_source_sha: 6ff923b4c01c0fac47edf0eeff6dc8c71e084eca
...
2026-07-20 기준. "자동화 테스트"는 test runner가 실행하는 pytest 또는 committed mock을
...
Azure Event Hubs가 설정되면 local factory는 실제 bus에서 전용 local consumer group으로 15개
agent를 모두 시작합니다. Workflow definition은 deployment와 같은 enforce allowlist를 사용하며
각 ActionType은 authoritative promotion 및 risk gate의 적용을 받습니다. Thor는 developer
credential을 받지 않으며 privileged execution은 deployed Managed Identity runtime에 남습니다.
Scenario replay, seeded audit row, recording executor, VM-task fake, synthetic scheduler/cost data,
scope template, blast-radius fixture는 pytest 전용입니다.
...
- Interactive local은 authoritative Event Hubs transport가 있을 때만 15개 agent를 모두 시작하며
  recording 또는 in-memory executor를 binding하지 않습니다.
