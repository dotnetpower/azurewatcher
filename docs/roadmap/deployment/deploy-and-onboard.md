...
| 5 | **Event Grid inventory system topic + subscription + Diagnostic Settings** | global subscription event delivery / Log Analytics | send resource writes/deletes to `aw.inventory.raw` and platform diagnostics to the workspace | the global system topic owns the inventory UAMI, which has send-only access to the operational shard; the core sees Kafka only |
