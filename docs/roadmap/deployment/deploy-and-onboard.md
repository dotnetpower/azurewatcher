...
| 5 | **Event Grid inventory subscription + Diagnostic Settings** | subscription event delivery / Log Analytics | send resource writes/deletes to `aw.inventory.raw` and platform diagnostics to the workspace | Event Grid uses the inventory UAMI and the operational-shard endpoint; the core sees Kafka only |
