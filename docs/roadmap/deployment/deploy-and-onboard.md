...
| 5 | **Event Grid inventory system topic + subscription + Diagnostic Settings** | global subscription event delivery / Log Analytics | send resource writes/deletes to `aw.inventory.raw` and platform diagnostics to the workspace | Terraform adopts the one tracked subscription system topic or creates it when absent, then assigns the send-only inventory UAMI; ambiguous discovery blocks the plan |
