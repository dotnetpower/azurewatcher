...
- 프로비저닝 리소스 - **최소 비용 효율 세트** (전체 인벤토리 + 티어 결정은
  ...
  - **Private StorageV2 case-history account**에 Shared Key 비활성화, Blob versioning,
    soft delete, bounded version lifecycle, 전용 non-executor workload identity, private endpoint를
    적용합니다. Content-addressed case revision을 저장하고 PostgreSQL에는 rebuildable hot
    index만 유지합니다.
