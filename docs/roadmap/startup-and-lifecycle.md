...
Complements [deploy-and-onboard.md](deploy-and-onboard.md) (which handles provisioning) and
...
- **Cold-start metric**: the first event on a cold path MAY exceed the T0 latency budget while
  ...
  KPI dashboard ([goals-and-metrics.md](goals-and-metrics.md)).
...
- Every rule MUST default to **`effect: audit` (shadow)** regardless of severity. There is no
  ...
  fails the promotion gate ([rule-governance.md](rule-governance.md)).
...
Full design: [llm-strategy.md § Model Provisioning and Lifecycle](llm-strategy.md#model-provisioning-and-lifecycle).
...
| **D+7 → D+14** | measure per-rule shadow accuracy + false-positive rate; identify low-risk promotion candidates | shadow sample size and accuracy threshold per [goals-and-metrics.md](goals-and-metrics.md) |
...
1. Create the Teams **group-connected team** backed by `aw-approvers` for HIL A1 traffic
   ...
   ([channels-and-notifications.md#51-audience-derivation-channel-as-audience](channels-and-notifications.md#51-audience-derivation-channel-as-audience)).
2. Provision the five Entra security groups (`aw-readers`, `aw-contributors`, `aw-approvers`,
   ...
   ([user-rbac-and-identity.md#42-security-groups-slots](user-rbac-and-identity.md#42-security-groups-slots)).
...
The [autonomous rule discovery loop](rule-catalog-collection.md#autonomous-rule-discovery) is
