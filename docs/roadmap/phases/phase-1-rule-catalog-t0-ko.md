...
translation_source_sha: a5a735e892b1413d5daeaa8fbf6c1fd16be0f7e4
...
- **Inventory 어댑터 (Azure)** -
  [inventory 계약](../architecture/csp-neutrality-ko.md#5-인벤토리-계약--리소스-그래프) 의 Azure 구현:
...
[rule-catalog-collection-ko.md](../rules-and-detection/rule-catalog-collection-ko.md) 에 상세.
...
- 모든 종단 경로(위반, no-op, abstain, HIL-route) 가 감사 엔트리를 씀; 감사 완전성 단언.- 어떤 T0 판정도 발사되기 전에 **인벤토리 그래프가 채워짐**: 병렬 full-scan 이 원자적으로
  ...
  upsert 로 no-op. Scan 사이에는 Activity Log delta 가 durable cursor 아래 event topic 으로
  다시 진입합니다. Graph-dependent action 은 RiskGate 에서 active snapshot age 를 읽고
  freshness 가 unknown 또는 stale 이면 HIL 로 route 합니다.
## 의존성
