...
translation_source_sha: 6f37e00e5e2a134900182a534f36ca34d52cc763
...
17. [Capability bundle 등록](#517-capability-bundle-등록)
...
### 5.17 Capability bundle 등록

**사용 시점**: 하나의 fork 기능에 운영자용 discovery와 reasoning tool,
`ActionType`, `Workflow` binding이 함께 필요할 때 사용합니다. 인프라 provider 하나만
교체한다면 앞의 더 좁은 recipe를 사용하세요.

**Seam**: `CapabilityBundle`은 `Capability` 메타데이터,
`CapabilityBinding` 참조, reasoning-tool provider를 함께 묶습니다.
`fdai.composition.install_capability_bundle(...)`로 설치합니다. Installer는 새
`Container`를 반환하며 원본은 변경하지 않습니다. `ActionType` 또는 `Workflow`
binding은 typed reference일 뿐이며 대상을 직접 호출하지 않습니다.

**바인딩 방법**:

1. Upstream과 fork의 tool, ActionType, Workflow 카탈로그를 함께 로드합니다.
2. Fork 소유 provider와 `CapabilityBundle`을 구성합니다.
3. 로드한 카탈로그 객체를 `install_capability_bundle`에 전달합니다.
4. Azure 모드에서는 반환된 container를 `wire_azure_container`에 전달합니다.
   설치된 reasoning-tool provider는 자동으로 포함됩니다.
...
capability를 resolve하는지, 잘못된 참조가 provider I/O 전에 실패하는지, provider
출력이 tool artifact의 output 계약을 만족하는지 확인합니다. 변경 capability는 shadow
mode로 유지하고 resolve된 대상을 직접 호출하지 말고 일반 risk-gate와 executor 경로로
테스트하세요.

**Anti-pattern**:

- 동일 provider를 bundle과 `AzureWireOverrides.tool_providers` 양쪽에 등록합니다.
  중복 id는 시작 오류입니다.
- Bundle을 변경 작업용 범용 함수 dispatcher로 사용합니다. 변경은 control loop가
