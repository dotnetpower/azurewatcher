...
| **Workload SLO / error budget** | **새 subsystem.** § 3.3 참조. | 컨트롤 플레인 SLO 는 존재 ([deployment.md § 157](../deployment/deployment.md)); incident 우선순위를 rank 하는 workload-facing SLI/SLO/burn-rate 추상화는 부재. 두 identity 를 혼동하지 않도록 컨트롤 플레인 SLO 와 분리 유지. |
...
| DORA metric ingestion (change-failure-rate, deploy-frequency) | Deferred. | MTTR + lead time 은 이미 [goals-and-metrics.md](../architecture/goals-and-metrics-ko.md) 에 존재; 부재한 두 piece 는 P2 스코프 밖의 git-history reader 가 필요. |
...
| Monitoring / alerting (외부 signal ingestion) | Partial | `core/detection/` correlation 은 ship; § 3.2 metric seam 은 SLO burn-rate 와 detection anomaly(`MetricSeriesSource`)에, log / trace seam 은 RCA telemetry grounding(`TelemetryEvidenceGatherer`)에 소비됨; real vendor adapter 만 남음 |
