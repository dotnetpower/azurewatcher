...
translation_source_sha: 8d29ffc6c2b3d7fba015ffc85880ef2962a2efd0
...
| `overrides` | Override → Rule (M:1) | override가 이 규칙 대상([rule-governance-ko.md](rule-governance-ko.md#override) 참조) |
...
[rule-catalog-collection-ko.md](rule-catalog-collection-ko.md) 의 규칙 스키마는 파이프라인이
...
모든 런타임 스텝은 submission-criteria 계약 있는 **온톨로지 액션**. 대부분은 결정론; 하나만이
프론티어 LLM 호출.

| ActionType | 레이어 | 비용 | 전제조건 |
|------------|--------|------|---------|
| `evaluate` | L1 (T0) | 순수 함수, 인메모리 OPA/Rego | 규칙의 `applies_to` 가 Resource와 매칭; `check_logic` 컴파일 |
| `simulate` (what-if) | L1 (T0) | 선언적 상태에 대한 순수 함수 | 리소스 상태 스냅샷 이용 가능 |
| `reuse` | L2 | O(1) 인덱스 SELECT | 학습된-액션 저장소의 `(signature, rule_id, catalog_version)` hit |
| `similarity` | L3 (T1) | 임베딩 1 + pgvector kNN | 이웃의 컨텍스트 호환성 검사 통과 |
| `cache-hit` | L4 | O(1) 키 lookup | TTL과 카탈로그/모델 버전 내 서명 매칭 |
| `reason` | L5 (T2) | 프론티어 LLM (primary + secondary; 불일치 시 escalated) | quality gate가 출력에 대한 권위 |
| `remediate` | risk-gate ⇒ delivery | draft PR emission | 액션이 policy-as-code로 검증 |
| `escalate` | risk-gate ⇒ ChatOps | HIL 요청 | 어떤 저렴 레이어도 케이스 해결 못함 |
| `abstain` / `deny` | 어떤 레이어 | 감사된 no-op | verifier가 거부하거나 grounding 실패 |

`reason` 만이 LLM 호출. 다른 모든 액션은 결정론이며 마이크로초-밀리초에 실행.
...
기존 표면([deploy-and-onboard-ko.md](deploy-and-onboard-ko.md#azure-resource-inventory-minimum-set))
...
| `Link` 인스턴스(Signal→Resource, Finding→Finding, ...) | 런타임 + 감사 | **PostgreSQL** | `ontology_link` |
