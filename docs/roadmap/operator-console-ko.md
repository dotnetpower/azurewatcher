...
오퍼레이터 콘솔은 **자율 SRE agent가 아니다**. FDAI의 판단 authority
...
| Multi-signal RCA | LLM이 log / metric / trace 상관관계 | 시스템이 ingest 에서 이미 상관관계 완료; 오퍼레이터는 *"왜 X로 결정했어?"*를 물음 |

추상적으로 어느 접근이 "더 좋다"는 없다; 서로 다른 제품이다. 오퍼레이터
콘솔은 FDAI의 `deterministic-first` 원칙과 rule-catalog collection
계약에 fit 하는 것.

### 1.2 공유 glossary에 추가된 어휘
...
Month-1 추가는 콘솔을 Azure SRE Agent의 "Autonomous Incident Response"
서사에 가깝게 만드는 양보 — 하지만 여전히 **이미 correlate 된** 결과를
surface; correlator는 Layer 1에 살고, narrator 안에 살지 않는다.
...
Chat, Azure SRE Agent 자체) 에게 expose MAY; upstream surface는 wire
