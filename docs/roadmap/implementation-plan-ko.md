...
AIOpsPilot의 다음 작업 트랜치에 대한 순서 확정 문서. 2026-07-06자 설계
결정("표준 세트")과 그로부터 도출되는 단계별 웨이브 플랜을 통합해 담았다.
...
- **납품 약속이 아님**: 이 문서는 조정용 기록이다. 각 PR은 자체 exit
  gate와
...
> 포크는 `core/` 편집 없이
> [project-structure.md](project-structure.md)의 DI seam으로만 조정한다.
...
## 2. 표준 세트 (2026-07-06 확정 결정)

각 결정은 스코프가 있고 테스트 가능하며, Wave F 시작 전에 스키마 수준에서
협상 불가다. 하위 PR에서 아래 결정 중 어느 하나에서라도 이탈하려면 먼저
이 문서를 변경해야 한다 (docs-first;
...
- [action-ontology.md § 9](action-ontology.md#9-audit-contract) 의
...
- [execution-model.md § 5.4](execution-model.md#54-executor-selection-at-dispatch)
  ...
  (속도를 위해 올릴 수 없음).
  [execution-model.md § 2.7](execution-model.md#27-combining) 의 절대
  올리지 않는 규칙은 유지된다.
...
- **F1** ActionType 스키마 확장.
  [action-ontology.md § 2](action-ontology.md#2-schema)의 새 필드:
...
- **F3** 배송된 16개 ActionType YAML
  ([action-ontology.md § 3.1](action-ontology.md#31-remediation)) 을
...
- **F4** `RiskTable`과 `risk-classification.yaml` cross-check로
  ...
  불변식).
- **F5** [action-ontology.md § 7.5](action-ontology.md#75-precedence)
...
- **F7** `ControlLoop`와 `composition.py`가 모든 dispatch를
  ...
  ([execution-model.md § 3](execution-model.md#3-unified-riskgate)) 로
...
F 뒤. [operator-console.md § Day 1](operator-console.md#day-1-this-session)
구현.
...
D1 뒤. [operator-console.md § Week 1](operator-console.md#week-1) 과
...
W1 뒤.
[execution-model.md § Week 2](execution-model.md#week-2) 구현.

- **W2.1** [action-ontology.md § 3.2](action-ontology.md#32-ops) 의
...
- **W2.2** [action-ontology.md § 3.3](action-ontology.md#33-governance)
...
- **W2.3** `src/aiopspilot/core/executor/direct_api.py`의 Direct-API
  ...
  :class:`~aiopspilot.core.executor.executor.ShadowExecutor`를 미러링:
...
W2 뒤. [execution-model.md § Month 1](execution-model.md#month-1) 과
[operator-console.md § Month 1](operator-console.md#month-1) 구현.
...
[assurance-twin.md](assurance-twin.md) 구현.
...
sign-off 대기 중인 상시 결정. 각각 특정 웨이브를 블록한다. 나열된
결정이 해결되기 전에는 그 웨이브가 시작되지 않는다.

- **OD-C1 (D1 블록)** 나레이터 프롬프트 카탈로그 경로:
...
- **OD-C2 (W1 블록)** `operator_memory` 스키마 sign-off (R6에 따라
  스키마는 감사 `action_kind` 모양; 뷰 스키마가 그 뒤를 따름).
- **OD-C3 (W1 블록)** 활성 break-glass 부여가 no-self-approval 불변식을
  paired-approver-only로 좁히는지 여부. 소유자:
  security-and-identity 문서 작성자.
- **OD-C4 (D1 블록)** CLI REPL 히스토리 경로와 보존. 제안:
  `~/.aiopspilot/console-history.jsonl`, 10 MiB 상한, 쓰기 전 redact.
- **OD-P1 (W2 블록)** Axis A용 비용 estimator API 표면. 소유자:
  Cost Governance vertical.
- **OD-P2 (M1 블록)** Month 1 gate에서 Azure Monitor 바인딩과 함께
  배송할 스타터 프로브 vs `NoOpBlastProbe`로 남을 것.
...
- [action-ontology.md](action-ontology.md) - R1/R2/R7이 랜딩하는
  스키마.
- [execution-model.md](execution-model.md) - R1이 제약하는 RiskGate.
- [operator-console.md](operator-console.md) - R2와 R3가 다듬는
  콘솔 모양.
- [assurance-twin.md](assurance-twin.md) - R4가 프리미티브를 공유하는
  Twin 서브시스템.
- [deployment-preflight.md](deployment-preflight.md) - R4가
  프리미티브를 공유하는 Preflight 서브시스템.
- [prompt-composition.md](prompt-composition.md) - R3가 통합하는 LLM
