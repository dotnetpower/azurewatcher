...
translation_source_sha: 5dd06e046b177d9edb367178d904a66e52c71dde
...
- 이벤트 기반 코어는 §5 와 §7 의 topic / object-type ownership 테이블을 읽고
  schema-validated pub/sub 를 wire 한다.
- 오퍼레이터 콘솔 ([operator-console.md](operator-console-ko.md)) 은 §7.3 과
  §7.5 를 읽고 자연어 질문을 per-user context 로 primary agent 에 라우팅한다.
- 룰-카탈로그와 executor ([action-ontology.md](action-ontology-ko.md),
  [execution-model.md](execution-model-ko.md)) 는 §8 을 읽고 각 ActionType 을
  initiator / judge / approver / executor / auditor 에 바인딩한다.
- 포크는 §11 을 읽고 어느 seam 이 열려 있고 (topic subscription, config
...
- **Deterministic-first, LLM-capable.** 모든 에이전트는 자기 bindings 로
  ...
  용도로만 예약된다 (§9). LLM 사용은 capability 이지 default 가 아니다.
- **Two-port 모델.** 모든 에이전트는 machine 트래픽용 typed pub/sub port 와
  사람 / 다른 에이전트용 conversational port 를 노출한다 (§7).
- **Single-writer, multi-reader topics.** 각 object type 은 정확히 하나의
  owner agent 만 publish 하고, 누구나 subscribe 할 수 있다 (§7.1).
...
- **판테온은 upstream 에서 고정.** 15개 에이전트 세트, 조직도, 역할 배정은
  잠겨 있다. 포크는 config seam (§11) 을 통해 동작을 커스터마이즈한다 -
...
- 각 도메인은 설정된 **가중치**를 가진다(기본은 우선순위 순서
  ...
  fork config가 재정의). 점수는 `weight * impact`.
...
| Thor | Responder | 2 | ActionRun, ActionAttempt | (dispatch 만; 직접 소유 없음 - §8.1) | no |
...
| Thor | execution-path health check, retry-strategy 캐시 warmup | verdict dispatch, rollback trigger, rate-limit 강제 | high-risk action pre-flight simulation | 1 (Cost-aware remediation), 2 (Predictive scale) |
| Forseti | rule-cache 리프레시, retrospective what-if batch, verdict coherence self-test | 이벤트 판단 (T0/T1/T2), domain_conflict emit, SecurityEvent emit | novelty drift 감지 (T0 vs T2 mix) | 1, 2, 5 (Security escalation), 8 (Judgment coherence) |
| Huginn | source health check, dedup window 유지, backpressure signal | 정규화 + dedup + correlate + publish | 적응형 스키마 학습 (T1 clustering, off-path) | 모든 워크플로우에 feed |
| Heimdall | anomaly baseline 업데이트, forecast 리프레시, external-actor 리스트 리프레시, agent-health probe | anomaly detect, drift detect, SecurityEvent correlate, notify_admin | multi-signal 다신호 상관 | 1, 2, 3 (DR drill), 5, 7 (Agent health), 9 (Rollback rehearsal) |
| Vidar | rollback-path 검증, DR readiness score, recovery-time SLI | perform_rollback, dr_failover | rollback rehearsal (shadow) | 3, 9 |
| Var | approval SLA 모니터, approver 가용성 tracking | HIL 카드 제시, quorum 강제, timeout / escalation | approval provenance 기록 | 4 (Override -> Discovery), 5 |
| Bragi | 만료 세션 정리, UserPreference index 리프레시 | NL routing, multi-agent aggregation, NL 렌더링 | intent classifier 재학습 (T1, off-path) | 7, 10 (Retrospective what-if) |
...
| Mimir | rule-source 폴링, regression suite, deprecation cycle | rule promote / revoke, cache-invalidation broadcast | freshness-score, stale-rule 감지 | 4, 6 (Handoff -> Capability), 8 |
...
([goals-and-metrics.md](goals-and-metrics-ko.md)) 에 이 metric 을 emit
...
| Huginn | 이벤트 처리 지연 p99, dedup 정확도 | 스키마 매칭 실패율 |
...
properties:
  ...
  initiates: [ActionType]          # propose 가능 (§8.1)
  subscribes: [Topic]              # typed-port 구독
  publishes: [Topic]               # typed-port 발행
  question_domains: [string]       # NL query 카테고리 (§7.3)
  owns_code_paths: [glob]          # self-introspection 용 RAG 범위 (§9)
...
Bragi 는 라우터이지 답변자가 아니다. 라우팅은 deterministic-first:

1. **T0 keyword / regex 매칭.** Intent 토큰을 `Agent.question_domains` 와
   비교. Domain specificity, 참조된 object type 의 ownership, 상호작용
   recency 로 점수화.
2. **T1 embedding similarity.** T0 가 abstain 하면 과거 해결된 query 와
   similarity 매칭; 여전히 deterministic ranking, LLM 없음.
3. **T2 intent classification.** T0/T1 모두 abstain 하면 LLM 이 intent 를
   분류하고, Bragi 는 분류된 intent 로 scoring 을 재실행.
4. **Handoff.** Scoring margin 이 여전히 임계값 미만이면
   `HandoffEscalation` 발행 (§7.4). 시스템은 추측 대신 GitHub issue 를
   생성한다.
...
에이전트가 소유 데이터, T0, T1, T2 (§9 의 LLM policy 에 따라) 로 요청을
...
- **Learner 경계.** Norns 는 기본적으로 metadata 만
  ...
  extraction 을 위해 turn body 노출; opt-out 이 shipping default.
...
(기존 스키마는 [action-ontology.md](action-ontology-ko.md), 아래 확장). 판테온의
...
compensating_action: ActionType?   # irreversible: true 가 아니면 필수
```
...
### 7.5 Compensating action 과 irreversibility

모든 mutation ActionType 은 `irreversible: true` 를 선언하지 않는 한 반드시
`compensating_action` 을 참조. 예:

| ActionType | compensating_action | irreversible |
|------------|---------------------|--------------|
| restart_vm | (self; restart 는 running 상태로 복귀) | false |
...
GitHub issue 로의 escalation (§7.4) 은 그 자체가 `ActionType` 이므로 동일
...
Fork 는 아래 ActionType 필드를 override 할 수 있다:

- `preconditions`, `stop_conditions`
- `default_mode` (엄격에서 느슨 방향만 불허: fork 는 enforce -> shadow 로
  조용히 이동 불가, 하지만 promotion gate 통과 시 shadow -> enforce 는 가능)
- `params.schema` (추가만, 삭제 불가)
- `promotion_gate` (강화만, 약화 불가)

Fork 는 아래를 override 할 수 **없다**:

- `executor`, `judge`, `approver`, `auditor`, `initiators`
- `compensating_action`, `irreversible`, `rollback_contract`

이들은 판테온 안전 경계이다. Override lint 는
`rule-catalog/action-types-overrides/` 에 CI 로 실행되고 잠긴 필드가
나타나면 merge 를 실패시킨다.
...
모든 에이전트의 conversational port 는 LLM 을 사용하여 소유 데이터와
`owns_code_paths` 로 소유하는 코드 (RAG) 위에 자연어 응답을 render. 이는
에이전트가 자기 자신을 introspect 하는 유일한 경로이다.
...
병합. Fingerprint 스킴은 §7.4 dedup 패턴을 재사용.
...
- **Compensating action 없는 action.** 모든 mutation 은
  `compensating_action` 또는 `irreversible: true` + HIL quorum 과 함께
  배포. 제3의 옵션은 없다.
...
| 학습 주제 | 읽기 |
|----------|------|
| ActionType 스키마와 기존 action 인벤토리 | [action-ontology.md](action-ontology-ko.md) |
| 통합 RiskGate, executor path, audit block | [execution-model.md](execution-model-ko.md) |
| Bragi 를 호스팅하는 conversational 표면 | [operator-console.md](operator-console-ko.md) |
| §9 가 참조하는 RBAC 역할 | [user-rbac-and-identity.md](user-rbac-and-identity-ko.md) |
| §9.3 이 참조하는 ChatOps 채널 라우팅 | [channels-and-notifications.md](channels-and-notifications-ko.md) |
| Rule 과 policy 가 Forseti 를 feed 하는 방식 | [rule-catalog-collection.md](rule-catalog-collection-ko.md), [rule-governance.md](rule-governance-ko.md) |
| Fork 경계와 DI seam | [downstream-fork-guide.md](downstream-fork-guide-ko.md) |
