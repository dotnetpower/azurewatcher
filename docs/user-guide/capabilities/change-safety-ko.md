...
- **드리프트 감지와 교정.** 선언된 상태에서 벗어난 구성은 감지 및 분류된 뒤 자동
  교정되거나 검토 대상으로 전달됩니다.
- **고위험 변경은 사람 승인을 기다립니다.** 리스크 게이트가 저위험 변경을 자동 병합으로,
  고위험 변경을 human-in-the-loop (HIL) 승인으로 라우팅합니다.
- **감사와 롤백은 기본 제공됩니다.** 액션은 remediation pull request로 전달되므로,
