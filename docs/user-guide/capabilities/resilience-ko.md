...
translation_source_sha: 7c1446158347fb9059180bd19a00b9a7488ea1c0
...
맞춰 훈련하며, blast radius가 제한된 카오스 실험을 실행하고, 이전에 확인한 실패 패턴을
...
1. **차이 찾기.** 예약된 작업이 중요 데이터베이스의 `point-in-time-restore` 미지원 구간
  같은 회복탄력성 문제를 감지하고 finding을 생성합니다.
2. **훈련 예약.** 에이전트가 지정된 훈련 시간대 안에 연계 복원 훈련을 예약하며,
  라이브 트래픽을 대상으로 범위 제한 없이 실행하지 않습니다.
3. **blast radius 안에서 실행.** 훈련은 범위, 배치, 속도 한도 안에서 실행됩니다.
...
출시되며, 측정된 정확도가 기준을 충족한 뒤에만 enforce로 승격됩니다.
