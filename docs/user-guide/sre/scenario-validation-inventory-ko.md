...
> answer grounding을 검증합니다. Conversational safety corpus이며 SRE 운영 scenario set이
...
| ID | Scenario | 유형 | 검증 |
|----|----------|------|------|
| S1 | AKS pod kill | Fault | Shadow coverage. `aks-pod-kill` live enforce |
| S2 | AKS pod CPU stress | Fault | Shadow coverage. `aks-pod-cpu-spike` live enforce |
| S3 | AKS pod network latency | Fault | Shadow coverage. `network-rtt-delay` live enforce |
| S4 | AKS HTTP abort | Fault | Shadow coverage. `aks-http-abort` live enforce |
| S5 | VM CPU stress | Fault | Shadow coverage. `vm-cpu-stress` live enforce |
| S6 | VM memory stress | Fault | Shadow coverage. `vm-mem-stress` live enforce |
| S7 | VM network latency | Fault | Shadow coverage. `network-rtt-delay` live enforce |
| S8 | MySQL CPU-credit exhaustion | Fault | Shadow coverage. `mysql-cpu-pressure` live enforce |
| S9 | TPM pressure에 따른 Azure OpenAI 429 | Fault | Shadow coverage. `aoai-tpm-throttle` live enforce |
| S10 | Application Gateway backend first-byte latency | Fault | Shadow coverage. `network-rtt-delay` live enforce |
| S11 | Dependency outage cascade | Fault | Shadow coverage. `appgw-backend-failure` live enforce |
| S12 | Bad deployment와 rollout stall | Fault | Shadow coverage. `aks-bad-deploy` live enforce |
...
|-------------|--------|-----------|---------------|-------------|
| `change.drift-manual-portal-edit.003` | Change | T0 | HIL | 없음 |
| `change.nsg-allow-any-inbound.002` | Change | T0 | HIL | 없음 |
...
| `dr.chaos-experiment-novel.003` | DR | T2 | HIL | 없음 |
| `dr.replica-lag-degraded.001` | DR | T1 | HIL | 없음 |
| `finops.right-size-vm-high-monthly.002` | FinOps | T0 | HIL | 없음 |
...
| `hil_unknown_event_triage` | 구체적인 resource가 있어 HIL |
| `abstain_no_resource_target` | actionable target이 없어 abstain |
...
| HIL request | Pending approval ticket 정확히 1개 |
| Deny | Approval 또는 execution에 도달하지 않음 |
| Mixed stream | Verdict와 dispatch count가 같음 |
| Duplicate delivery | Dispatch 1회, duplicate 1회 측정, 이중 실행 없음 |
| Empty 또는 junk event | Abstain하고 downstream action 없음 |
...
- **Shadow covered**는 변경 없이 detection, routing, RCA, response, safety mapping이
  존재한다는 뜻입니다.
- **Live enforce validated**는 일회용 substrate에서 injection, expected detection,
...
| Finding과 forecast를 검증하는 방법 | [관측성, 감지, 예측](observability-detection-and-forecasting-ko.md) |
