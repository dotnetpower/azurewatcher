...
관련 리소스 이벤트, telemetry finding, 변경을 안정적인 멤버십과 시간 순서를 가진
하나의 인시던트로 연계합니다.
...
제안을 trust-router, risk-gate, 승인 정책 뒤에 둡니다.

예시: 오류율 알림 -> 조사가 최근 배포를 연계 -> RCA가 변경과 telemetry를 인용 ->
대응 계획이 rollback을 제안 -> HIL 승인이 제안을 액션 파이프라인에 다시 넣을지 결정.
...
- **Telemetry 시스템**: metric, log, trace provider는 증거를 공급하지만 두 번째 실행
  경로가 되지 않습니다.
- **Git과 ChatOps**: remediation pull request가 변경을 전달하고, Teams 또는 Slack이
  승인과 운영 알림을 전달합니다.
...
1. **관찰하고 연계합니다.** 이벤트와 finding을 정규화 및 중복 제거하고, 관련 멤버를
   하나의 인시던트로 묶습니다.
...
| 시나리오 증거 | [시나리오 검증 인벤토리](scenario-validation-inventory-ko.md) | Demo 18개, live enforce 10개, frozen replay 9개, catalog scenario 132개 |
