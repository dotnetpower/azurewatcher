...
Resource analyzer는 provider evidence를 읽고 structured finding을 반환합니다. Coordinator는
timeline, correlation, optional root-cause hypothesis, prioritized recommendation을 만듭니다.

Report는 read-only입니다. Remediation을 지정하는 recommendation도 제안일 뿐이며 typed
...
| 섹션 | 답하는 질문 |
|------|-------------|
| Findings | 각 resource analyzer가 무엇을 관찰했나요? |
