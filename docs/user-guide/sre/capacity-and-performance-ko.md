...
Telemetry가 없거나 오래되면 unavailable 또는 abstained evidence가 됩니다. Demand가 0이라고
가정하지 않습니다.
...
역할을 유지하며 Forseti와 risk-gate가 설정된 precedence와 autonomy ceiling을 적용합니다.

Advice가 충돌하면 Forseti가 cross-vertical arbitration request를 생성합니다. Odin은
rule catalog의 버전 관리되는 priority policy를 적용하고, Forseti가 verdict를 만들기 전에
...
제공할 수 있습니다. Arbitration result는 증거이며 risk-gate ceiling을 완화할 수 없습니다.
...
검사 -> risk-gate가 shadow, approval, promoted execution 중 하나를 결정.

## Scaling proposal 흐름

1. Detector 또는 scheduled evaluation이 capacity finding을 생성합니다.
2. Finding을 workload SLO, current change, incident와 연계합니다.
...
| Floor, batch, rate limit 충족 | Risk-gate로 진행 | Deny 또는 scope 축소 |
