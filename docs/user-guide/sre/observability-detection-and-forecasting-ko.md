...
translation_source_sha: 96f9709e327ab67eb85be8ac3218bd748b957512
...
quorum만큼 발생할 때만 finding을 생성합니다. Quorum 미만에서는 abstain합니다. Quorum
이상이면 member의 범위와 결합 magnitude에 따라 severity를 높일 수 있지만, 결과는 계속
shadow-mode finding으로 유지됩니다.
...
유한하지 않은 입력, sample 부족, 잘못된 ratio 입력은 finding을 만들지 않고 보류합니다.
Threshold와 metric binding은 catalog data에 남으므로 배포 환경에서 evaluator를 변경하지
않고 조정할 수 있습니다. 모든 recipe는 shadow mode에서 시작하고 trust routing 전에 안정적인
...
| Cold 또는 stale history | Abstain 또는 보류 | 증거가 불충분함 |
...
승격 전에 forecaster는 알려진 과거 위반을 backtest하고 shadow mode에서 설정된 정확도
...
5. Correlation ID를 따라 RCA, verdict, action proposal, audit row를 확인합니다.
