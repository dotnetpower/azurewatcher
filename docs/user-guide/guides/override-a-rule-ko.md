...
자동 품질 게이트 위에 앉는 **스코프된 override**를 지원합니다.
...
- **`disabled`** - 그 스코프에서 규칙 실행이 멈춥니다. 탐지는 shadow로 계속
  실행됩니다 (감사 로그는 규칙이 *would have flagged*한 것을 계속 기록), 발견
  루프가 반복되는 override 패턴을 잡을 수 있도록.
