...
AIOpsPilot은 자율 클라우드 운영 컨트롤 플레인입니다. 운영 이벤트 중 반복 가능한 다수를
...
AIOpsPilot은 하나의 이벤트 기반 코어 위에 세 개의 버티컬을 얹습니다. 각 버티컬은 고유
...
훈련을 스케줄링 -> RPO/RTO 목표에 대해 복원 성공 -> 감사 엔트리로 기록.
...
예시: 비용 이상 탐지기가 캐시 티어 과잉 프로비저닝에서 발화 -> T0 규칙 매칭 -> 2주
...
flowchart TB
  ...
  Q4 -->|예| OK[AIOpsPilot 이 적합.<br/>Phase 0 부터 시작.]
```

- 운영자가 반복적으로 승인·롤백하는 클라우드 설정 이벤트(드리프트, 비용 회귀, 정책
  위반)에 실제 시간을 쓰고 있다.
- 인프라가 IaC와 policy-as-code로 표현되어 있다(또는 그 방향으로 가고 있다).
- 자율성 이득을 재기 위한 베이스라인이 있거나 만들 수 있다. AIOpsPilot은 측정된 짝 없이
  배수를 주장하지 않습니다.
...
- **일회성·비반복 인시던트**: AIOpsPilot의 이득은 반복 가능한 다수를 자동화하는 데
