...
FDAI는 안전한 다수의 이벤트를 스스로 해결하지만, 위험한 소수는 사람을 위해
멈춥니다. 이 페이지는 **시스템이 여러분에게 어떻게 도달하는가**를 설명합니다 -
어떤 채널이 승인 요청을 나르는지, 유출된 메시지가 왜 유효한 승인이 될 수 없는지,
그리고 승인이 타임아웃되거나 모든 채널이 다운되면 무슨 일이 벌어지는지.

오퍼레이터 콘솔은 **읽기 전용**입니다. 상태와 대기 중인 승인 큐를 렌더링하지만
특권 호출은 하지 않습니다. 여러분은 콘솔의 버튼을 눌러 승인하지 않습니다. 승인은
여러분이 이미 쓰는 채널(Teams, Slack)이나 remediation PR을 통해 전달됩니다 -
결코 콘솔의 신원으로는 아닙니다.
...
| 카테고리 | 방향 | 예시 | 누가 나를 수 있는가 |
|----------|------|------|---------------------|
| **A1 - 승인** | 여러분이 결정하고, 그 결정이 돌아옴 | 고위험 액션 승인, enforce 승격, 면제(exemption), 오버라이드 | 신원이 검증된 채널만 |
...
정보를 나를 뿐 권한을 나르지 않으므로 신뢰도가 낮은 채널로도 흐를 수 있습니다.

## 승인이 여러분에게 도달하는 방식

리스크 게이트가 액션을 **HIL**로 분류하면([risk-tiers-ko.md](risk-tiers-ko.md)
...
채널은 여러분의 신원을 끝에서 끝까지 증명할 수 있을 때만 승인을 나를 수 있습니다.
...
| **이메일** | 아니오 | A2, A4만 |
| **웹훅** | 아니오 | A2만 |
| **PagerDuty / Opsgenie / SMS** | 아니오 | A2만 - 페이징 레인 |
...
FDAI는 사용자별로 수신자 목록을 만들지 않습니다. **각 채널이 곧 오디언스**이며,
...
- **안전한 다수는 자동 해결**됩니다 - stop-condition, 롤백 경로, blast-radius
  제한, 감사 항목을 갖추고 - 메시지가 여러분에게 아예 도달하지 않습니다.
- **위험한 소수는 여러분을 위해 멈추고**, 여러분은 이미 쓰는 채널에서 결정합니다.
  거부와 타임아웃은 둘 다 no-op이며, 둘 다 감사됩니다.
