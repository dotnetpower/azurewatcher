...
모든 제안 액션은 이벤트, 대상 리소스, 환경, 액션이 명시한 blast radius로부터
...
- **AUTO** - 직접 실행해도 안전할 만큼 낮은 리스크. 감사 로그는 여전히 누가, 무엇을,
  언제, 왜를 기록합니다.
- **HIL** - 운영자가 승인해야 합니다. FDAI는 실행을 멈추고 Teams, 재인증을 구성한
...
먼저 리스크 분류 테이블을 평가합니다. 규칙은 deny, HIL, auto, 기본 HIL 순서로
실행하며 처음 일치한 규칙이 기준 판정이 됩니다.
...
| 인벤토리 그래프가 오래됨 | DENY | 오래된 관계로 blast radius를 계산할 수 있음 |
