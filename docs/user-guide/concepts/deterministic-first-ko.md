...
translation_source_sha: 62ae716ee4f2a668a4606e8cbac5da2b781b536c
...
T0 규칙이 일치한 경우를 포함해 모든 판정은 액션 실행 전에 risk-gate를 통과해야 합니다.
...
- **T0 - 결정론 (목표 이벤트의 ~70-80%)**. policy-as-code (OPA), 체크리스트,
  ...
  카탈로그 우선순위를 적용하고, 우선순위가 같은 충돌은 HIL로 전달합니다.
...
보류하면 자율 변경 없이 HIL로 전달합니다. 예상하지 못한 오류도 같은 안전한 경로를
따르며 감사 추적에 기록됩니다.

## T2가 증명해야 하는 것

T2는 누락된 규칙을 모델의 자신감으로 대체하는 권한이 아닙니다. T2 제안이 risk-gate에
...
모델 불일치는 유용한 증거입니다. FDAI는 경쟁하는 제안을 보존하고 HIL로 전달합니다.
...
- **T2**: 모델 식별자, 구조화된 제안, 합의 결과, verifier 검사, 근거 인용,
  판단 보류 사유.
- **risk-gate**: 일치한 리스크 규칙, 카탈로그 버전, 가장 엄격한 자율성 상한,
  최종 auto, HIL, deny 판정.
...
- enforce 이후 롤백과 정책 위반 누락 방지 지표.

같은 고정 시나리오 세트와 배포 기간을 기준으로 값을 비교하세요. T0 비율 증가는 false
negative, 롤백 비율, 정책 위반 누락이 함께 악화되지 않을 때만 의미가 있습니다.
