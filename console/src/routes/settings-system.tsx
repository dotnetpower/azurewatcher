...
export function SettingsIntegrationsRoute({ client, auth }: Props) {
  ...
  usePublishViewContext(
    () => ({
      ...
      headline: `Authentication ${authMode}; Read API ${client.readApiBaseUrl}`,
      ...
      <section class="settings-section" aria-labelledby="settings-identity-integration">
        ...
        <div class="settings-list">
          <SettingRow label={t("settings.entra")} hint={t("settings.entraHint")}>
            <StatusPill kind="success" label={authMode} />
          </SettingRow>
        </div>
      </section>
      <section class="settings-section" aria-labelledby="settings-delivery-integrations">
        ...
        <div class="settings-list">
          <SettingRow label={t("settings.githubApp")} hint={t("settings.githubAppHint")}>
            <StatusPill kind="neutral" label={t("settings.deploymentManaged")} />
          </SettingRow>
...
export function SettingsDiagnosticsRoute({ client, auth }: Props) {
  ...
  usePublishViewContext(
    () => ({
      ...
      glossary: composeGlossary([TERMS.humanRbac]),
      headline: `${authMode}; ${client.readApiBaseUrl}`,
      capturedAt: new Date().toISOString(),
      facts: [
        { key: "read_api", value: client.readApiBaseUrl, group: "runtime" },
        { key: "authentication_mode", value: authMode, group: "identity" },
        { key: "principal", value: auth.account?.username ?? "unavailable", group: "identity" },
      ],
      records: {},
    }),
    [auth, authMode, client],
  );
