...
function RetentionWidget({ widget }: { readonly widget: RenderedWidget }) {
  ...
  })}</tbody></table></div>{rows.length === 0 ? <p class="muted small">{t("workflow.process.noRetentionCohorts")}</p> : null}</section>;
}
...
function SplitGraphWidget({ widget }: { readonly widget: RenderedWidget }) {
  ...
  return <section class="process-widget-section" aria-labelledby={`${widget.id}-title`}><h3 id={`${widget.id}-title`}>{widget.title}</h3><div class="report-small-multiples">{panels.map((panel, index) => {
    ...
    return <article key={`${displayValue(panel["label"])}-${index}`}><strong>{displayValue(panel["label"])}</strong><svg viewBox="0 0 160 48" role="img" aria-label={t("workflow.process.trendAria", { label: displayValue(panel["label"]) })}><polyline points={sparkline(points, 160, 48)} fill="none" stroke="currentColor" stroke-width="2" /></svg><LabelList labels={asRecord(panel["labels"])} /></article>;
  })}</div>{panels.length === 0 ? <p class="muted small">{t("workflow.process.noSplitPanels")}</p> : null}</section>;
}
