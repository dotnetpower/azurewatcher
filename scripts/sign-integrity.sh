...
# done OFFLINE by anyone with the committed public key (scripts/check-integrity.sh).
#
...
#
# Usage:
#   scripts/sign-integrity.sh --gen-key   # one-time: create the Ed25519 keypair
#   scripts/sign-integrity.sh             # regenerate manifest (if needed) + sign
#   scripts/sign-integrity.sh --no-regen  # sign the manifest exactly as-is on disk
#
# Environment:
...
if [ "$gen_key" = "1" ]; then
  ...
  echo "  Next: scripts/sign-integrity.sh    # regenerate manifest + sign"
  exit 0
fi

if [ ! -f "$PRIVKEY" ]; then
  echo "sign-integrity: ERROR - private key not found at $PRIVKEY." >&2
  echo "  Create one first: scripts/sign-integrity.sh --gen-key" >&2
  echo "  Or point FDAI_INTEGRITY_KEY at your signing key." >&2
  exit 1
fi

if [ "$regen" = "1" ]; then
  "${python_cmd[@]}" scripts/gen-integrity-manifest.py --out "$MANIFEST"
fi
...
if openssl pkeyutl -verify -pubin -inkey "$PUBKEY" -rawin -sigfile "$raw_sig" -in "$MANIFEST" >/dev/null 2>&1; then
  ...
  echo "  signature: $SIG"
  echo "  public:    $PUBKEY"
  echo "  Commit all three. Verify anywhere offline: scripts/check-integrity.sh"
else
