...
(scripts/sign-integrity.sh); a downstream fork verifies it fully OFFLINE with
the committed public key (scripts/check-integrity.sh).
...
Usage:
  scripts/gen-integrity-manifest.py [--out PATH] [--check]
...
def main() -> int:
    ...
    if args.check:
        ...
        print(
            ...
            f"    scripts/gen-integrity-manifest.py --out {args.out}\n"
            "    scripts/sign-integrity.sh",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        ...
        "Now sign it: scripts/sign-integrity.sh"
    )
    return 0
