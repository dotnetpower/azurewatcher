...
#                       (scripts/sign-integrity.sh). A SIGNATURE failure is
...
#
# Usage:  scripts/check-integrity.sh
...
for f in "$MANIFEST" "$SIG" "$PUBKEY"; do
  if [ ! -f "$f" ]; then
    ...
    echo "  Upstream must generate + sign first: scripts/sign-integrity.sh --gen-key && scripts/sign-integrity.sh" >&2
    exit 2
  fi
done
...
# Upstream: advisory - the framework legitimately evolves here; re-sign.
{
  ...
  echo "  scripts/sign-integrity.sh"
} >&2
exit 0
