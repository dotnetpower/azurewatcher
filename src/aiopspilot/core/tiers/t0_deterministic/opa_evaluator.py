...
- Every module declares ``package aiopspilot.<derived path>`` matching its
  file location. Given ``rule.check_logic.reference == "policies/foo/bar.rego"``,
  the query is ``data.aiopspilot.foo.bar``.
...
:class:`~aiopspilot.core.tiers.t0_deterministic.AbstainEvaluator` explicitly;
...
from aiopspilot.shared.contracts.models import CheckLogicKind, Rule

_DEFAULT_TIMEOUT_SECONDS: Final[float] = 5.0
_PACKAGE_ROOT: Final[str] = "aiopspilot"
...
class OpaRegoEvaluator:
    ...
    def _derive_package(relative: Path) -> str:
        """Derive ``aiopspilot.<a>.<b>.<c>`` from ``<a>/<b>/<c>.rego``."""
