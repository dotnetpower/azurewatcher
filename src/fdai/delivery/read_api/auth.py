...
:mod:`fdai.core.rbac` and the concrete HTTP surface (Session I, the
...
:func:`fdai.core.rbac.resolver.decode_jwt_payload` and MUST NOT be
wired in production.
"""
...
- check ``aud`` matches ``api://<fdai-api-guid>``;
...
class Authenticator:
    ...
    :class:`~fdai.core.rbac.resolver.Principal` or raises
