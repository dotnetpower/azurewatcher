...
def _locale_directive(locale: str) -> str:
    ...
    return (
        ...
        "Use fluent standard spelling and silently proofread only narrator-authored prose for "
        "malformed words, accidental character sequences, duplicated fragments, "
        "and accidental language mixing. "
        "Keep every evidence value, id, number, tool output, code fragment, and column name "
        "verbatim; localise only narrator-authored surrounding prose."
    )
