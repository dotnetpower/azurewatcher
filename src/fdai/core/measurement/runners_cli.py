...
Terraform module. The job's ``FDAI_MEASUREMENT_MODE`` env var
...
.. code-block:: shell
    ...
    FDAI_MEASUREMENT_MODE=baseline python -m fdai.core.measurement.runners_cli
    FDAI_MEASUREMENT_MODE=growth   python -m fdai.core.measurement.runners_cli
...
- ``2`` — invalid ``FDAI_MEASUREMENT_MODE`` (fail-fast per
...
_LOGGER = logging.getLogger("fdai.core.measurement.runners_cli")

_ENV_MODE = "FDAI_MEASUREMENT_MODE"
...
async def _run_baseline() -> int:
    ...
    The full :class:`~fdai.core.measurement.runners.AutomatedBaselineRunner`
...
async def _run_growth() -> int:
    ...
    :class:`~fdai.core.measurement.runners.PatternGrowthIntakeRunner`
