 ...
 * `relPath` is always relative to the top-level `dir` of the walk (the mount
 * source), so a file nested in a subdirectory keeps its subdirectory prefix
 * (e.g. `concepts/foo.md`). `baseDir` carries that root through the recursion;
 * without it, `relative()` would resolve against each recursion directory and
 * silently flatten `concepts/` and `guides/` into the mount root - which
 * empties the autogenerated sidebar groups keyed on those directories.
 */
async function* walkMarkdown(dir, baseDir = dir) {
  ...
  for (const entry of entries) {
    ...
    yield { absPath: abs, relPath: relative(baseDir, abs) };
  }
}
