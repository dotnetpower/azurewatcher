...
translation_source_sha: 7686f76c6fcc37d7466e6babeda0eff374479509
...
policy-as-code에 대해 dry-run되고 blast-radius가 한정되며, 저위험은 자동 병합,
고위험은 human-in-the-loop(HIL)로 라우팅됩니다.
...
- **이벤트 버스**: Event Hubs Standard의 Kafka wire protocol. Native Azure
  ...
  봅니다.
- **CSP-neutral 설계**: 클라우드 접근은 provider 어댑터(OPA policy, Terraform IaC)
  뒤에 있습니다. Azure가 구현 대상이며, 비-Azure는 TBD이고 배송 약속이 아닌
  보존된 seam으로 취급됩니다.
...
2. **Route**: trust router가 confidence를 계산해 가장 낮은 충분 티어를 선택.
   T0 결정론(규칙 판정) -> T1 lightweight 재사용(해결된 인시던트와의 유사도) ->
   T2 추론(frontier LLM + verifier + mixed-model cross-check + policy grounding).
   T2 출력은 quality gate를 통과해야 실행 자격이 생깁니다.
3. **Gate and act**: risk gate가 auto / HIL / abstain / deny를 결정합니다. auto와
   승인된 HIL 액션은 remediation PR이 됩니다. 모든 terminal 경로(reject, timeout,
   abstain 포함)는 감사 엔트리를 씁니다.
